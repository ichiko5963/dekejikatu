...
1. NewsAPI.orgからAI関連ニュースを取得
   - **実行日のJSTでの日付範囲を指定**（その日の00:00:00から23:59:59まで）
   - 日本語クエリ優先、失敗時は英語クエリにフォールバック
   - `sortBy: 'popularity'`で注目度順に取得
   - 最大20件取得し、重複除外後に上位3件を選択
2. **重複排除機能**
   - 過去に送信したニュースURLを`data/ai-news-sent.json`に記録
   - 過去に送信していない記事のみを選択（最新100件まで保持）
   - **毎日必ず異なるニュースが配信されることを保証**
3. OpenAI APIを使用してニュースを要約
   - **実行日の日付を明記**（例：「本日2025年11月2日の」）
   - デジリューの口調で350〜650文字に要約
   - 要約失敗時はフォールバック形式で投稿
4. Discordに投稿

**メッセージ形式**:
- 導入: 「おはよう、デジリューだよ。本日2025年11月2日はAIの動きが気になる朝。さくっと行こっか。」（日付を明記）
- 本文: 箇条書き3件（タイトル、1行サマリ、リンク、インパクト）
- 締め: 「時間がない人は、最初の1本だけでもOK。今日もいい一歩にしようね ☕」

**重複防止の仕組み**:
- `data/ai-news-sent.json`に送信済みURLを記録
- NewsAPIから取得した記事をフィルタリングして、未送信のもののみを選択
- これにより、毎日必ず異なるニュースが配信される
